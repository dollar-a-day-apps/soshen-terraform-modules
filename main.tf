...
resource "aws_route53_record" "soshen" {
  ...
  type    = var.route53_record[count.index].type
  name    = var.route53_record[count.index].name

  alias {
    evaluate_target_health = var.route53_record[count.index].alias_evaluate_target_health

    # Accessing node.soshen.io via a browser will send the visitor to our load balancer
    zone_id = var.route53_record[count.index].alias_zone_id
    name    = var.route53_record[count.index].alias_name
  }
}
